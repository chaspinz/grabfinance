...
export default function App() {
  ...
  const StatCard = ({label,val,color,sub}) => (
    <div style={{background:"#0d1b2e",border:`1px solid ${color}25`,borderRadius:14,padding:"16px 18px",position:"relative",overflow:"hidden"}}>
      <div style={{fontSize:10,color:"#64748b",fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{label}</div>
      ...
      <div style={{position:"absolute",right:-8,top:-8,width:60,height:60,borderRadius:"50%",background:color,opacity:0.08}}/>
    </div>
  );

  const TT = {formatter:(v)=>fmt(v), contentStyle:{background:"#0d1b2e",border:"1px solid #1e3a5f",borderRadius:8,fontSize:11}};

  // ─── Render ────────────────────────────────────────────────────
  return (
    <div style={{fontFamily:"'Sora','Nunito',sans-serif",background:"#070d1a",minHeight:"100vh",color:"#e2e8f0",display:"flex",flexDirection:"column"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:#070d1a}::-webkit-scrollbar-thumb{background:#1e3a5f;border-radius:2px}
        .inp{background:#0a1628;border:1px solid #1e3a5f;color:#e2e8f0;border-radius:10px;font-family:inherit;font-size:13px;padding:10px 13px;width:100%;transition:border .2s}
        ...
        .nav:hover{background:#0d1b2e;color:#94a3b8}
        .nav.on{background:linear-gradient(135deg,rgba(14,165,233,.18),rgba(99,102,241,.12));color:#38bdf8;border:1px solid rgba(14,165,233,.2)}
        .card{background:linear-gradient(135deg,#0d1b2e,#0f2040);border:1px solid #1e3a5f;border-radius:16px}
        .tr{border-bottom:1px solid #1e3a5f;transition:background .15s}
        ...
        .toast{position:fixed;top:16px;right:16px;z-index:9999;padding:11px 18px;border-radius:12px;font-weight:600;font-size:12px;display:flex;align-items:center;gap:8px;box-shadow:0 8px 30px rgba(0,0,0,.6);animation:slIn .3s ease;max-width:300px}
        ...
        @media(max-width:768px){
          ...
          .botNav{display:flex;position:fixed;bottom:0;left:0;right:0;background:#080d1a;border-top:1px solid #1e3a5f;z-index:100;padding:4px 0}
      ...
      {/* Top bar */}
      <header style={{background:"#080d1a",borderBottom:"1px solid #1e3a5f",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontWeight:800,fontSize:15,color:"#f1f5f9",lineHeight:1.2}}>GrabFinance</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {/* Refresh */}
          <button className="btn" onClick={loadData} style={{padding:"6px 10px",background:"#0d1b2e",border:"1px solid #1e3a5f",color:"#94a3b8",fontSize:11}}>
            🔄
          </button>
        </div>
      </header>
      ...
      <div style={{display:"flex",flex:1}}>
        ...
        <aside className="sidebar" style={{width:200,background:"#080d1a",borderRight:"1px solid #1e3a5f",padding:"16px 10px",display:"flex",flexDirection:"column",position:"sticky",top:57,height:"calc(100vh - 57px)",overflowY:"auto"}}>
          ...
          <div style={{borderTop:"1px solid #1e3a5f",paddingTop:12,display:"flex",flexDirection:"column",gap:3}}>
            <div className="nav" style={{color:"#34d399"}} onClick={exportExcel}>📥 Export Excel</div>
          </div>
          <div style={{marginTop:10,padding:"8px 10px",background:"#0a1628",borderRadius:8,fontSize:9,color:"#475569",textAlign:"center",lineHeight:1.6}}>
        ...
        <main className="main" style={{flex:1,padding:"22px 24px 40px",overflowY:"auto",maxWidth:"100%"}}>
          ...
          {loading&&(
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {[...Array(3)].map((_,i)=>(
                <div key={i} style={{height:80,borderRadius:14,background:"linear-gradient(90deg,#0d1b2e 25%,#1e3a5f 50%,#0d1b2e 75%)",backgroundSize:"200%",animation:"shimmer 1.5s infinite"}}/>
              ))}
          ...
          {tab==="dashboard"&&(
            <div>
              <h1 style={{fontSize:22,fontWeight:800,color:"#f1f5f9",marginBottom:18}}>Dashboard 🏠</h1>
              ...
              <div className="grid32" style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:14,marginBottom:14}}>
                <div className="card" style={{padding:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>TREN BULANAN</div>
                ...
                <div className="card" style={{padding:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>KATEGORI PENGELUARAN</div>
                  <ResponsiveContainer width="100%" height={190}>
                    <PieChart>
              ...
              {/* Prediksi hari */}
              <div className="card" style={{padding:18}}>
                <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>🔥 PREDIKSI HARI RAMAI (berdasarkan rata-rata pemasukan)</div>
                ...
                <div style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}>
                  {dayPred.slice(0,3).map((d,i)=>(
                    <div key={i} style={{background:i===0?"rgba(56,189,248,.1)":"rgba(99,102,241,.08)",border:`1px solid ${i===0?"#38bdf840":"#6366f130"}`,borderRadius:8,padding:"5px 12px",fontSize:11}}>
          ...
          {tab==="pemasukan"&&(
            <div>
              <h1 style={{fontSize:22,fontWeight:800,color:"#f1f5f9",marginBottom:20}}>💰 Input Pemasukan</h1>
              <div className="grid31" style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:18}}>
                <div className="card" style={{padding:20}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:5,fontWeight:700,letterSpacing:.5}}>TANGGAL</label>
                      <input type="date" className="inp" value={fI.tanggal} onChange={e=>setFI({...fI,tanggal:e.target.value})}/>
                    </div>
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:5,fontWeight:700,letterSpacing:.5}}>URAIAN / KETERANGAN</label>
                    ...
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:5,fontWeight:700,letterSpacing:.5}}>JUMLAH (Rp)</label>
                  ...
                  <div style={{marginTop:18,padding:12,background:"#0a1628",borderRadius:10}}>
                    <div style={{fontSize:10,color:"#64748b",fontWeight:700,marginBottom:5}}>TOTAL PEMASUKAN</div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:20,fontWeight:800,color:"#34d399"}}>{fmtShort(stats.tI)}</div>
                    <div style={{fontSize:10,color:"#475569",marginTop:3}}>{pi.length} transaksi total</div>
                  </div>
                </div>
                <div className="card" style={{padding:20}}>
                  ...
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr style={{borderBottom:"2px solid #1e3a5f"}}>
                        {["Tanggal","Uraian","Jumlah",""].map(h=><th key={h} style={{textAlign:"left",padding:"6px 10px",color:"#475569",fontWeight:700,fontSize:10,letterSpacing:.5}}>{h}</th>)}
                      </tr></thead>
                      <tbody>
                        {[...pi].sort((a,b)=>b.Tanggal>a.Tanggal?1:-1).map(x=>(
                          <tr key={x.ID} className="tr">
                            <td style={{padding:"9px 10px",color:"#64748b",fontSize:11,whiteSpace:"nowrap"}}>{x.Tanggal}</td>
                    ...
                    {!pi.length&&<div style={{textAlign:"center",padding:40,color:"#475569",fontSize:13}}>Belum ada data pemasukan</div>}
                  </div>
                </div>
              </div>
            </div>
          ...
          {tab==="pengeluaran"&&(
            <div>
              <h1 style={{fontSize:22,fontWeight:800,color:"#f1f5f9",marginBottom:20}}>💸 Input Pengeluaran</h1>
              <div className="grid31" style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:18}}>
                <div className="card" style={{padding:20}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    ...
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:5,fontWeight:700,letterSpacing:.5}}>KATEGORI</label>
                    ...
                    <div>
                      <label style={{fontSize:10,color:"#64748b",display:"block",marginBottom:5,fontWeight:700,letterSpacing:.5}}>URAIAN</label>
                  ...
                  <div style={{marginTop:18,padding:12,background:"#0a1628",borderRadius:10}}>
                    <div style={{fontSize:10,color:"#64748b",fontWeight:700,marginBottom:5}}>TOTAL PENGELUARAN</div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:20,fontWeight:800,color:"#f87171"}}>{fmtShort(stats.tO)}</div>
                    <div style={{fontSize:10,color:"#475569",marginTop:3}}>{po.length} transaksi total</div>
                  </div>
                </div>
                <div className="card" style={{padding:20}}>
                  ...
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr style={{borderBottom:"2px solid #1e3a5f"}}>
                        {["Tanggal","Kategori","Uraian","Jumlah",""].map(h=><th key={h} style={{textAlign:"left",padding:"6px 10px",color:"#475569",fontWeight:700,fontSize:10,letterSpacing:.5}}>{h}</th>)}
                      </tr></thead>
                      <tbody>
                    ...
                    {!po.length&&<div style={{textAlign:"center",padding:40,color:"#475569",fontSize:13}}>Belum ada data pengeluaran</div>}
                  </div>
                </div>
              </div>
            </div>
          ...
          {tab==="laporan"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
                <h1 style={{fontSize:22,fontWeight:800,color:"#f1f5f9"}}>📋 Laporan Keuangan</h1>
              ...
              <div className="card" style={{padding:"12px 16px",marginBottom:18,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                <span style={{fontSize:10,color:"#64748b",fontWeight:700,letterSpacing:.5}}>FILTER:</span>
              ...
              <div className="grid3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:18}}>
                {[
                  ...
                  <div key={i} className="card" style={{padding:16,textAlign:"center"}}>
                    <div style={{fontSize:10,color:"#64748b",fontWeight:700,marginBottom:7,letterSpacing:.5}}>{s.l}</div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:20,fontWeight:800,color:s.c}}>{fmtShort(s.v)}</div>
                    <div style={{fontSize:10,color:"#475569",marginTop:3}}>{fmt(s.v)}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="card" style={{padding:18,marginBottom:18}}>
                <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>GRAFIK PERBANDINGAN BULANAN</div>
                <ResponsiveContainer width="100%" height={190}>
                  ...
                  <div key={ti} className="card" style={{padding:18}}>
                    ...
                    <div style={{maxHeight:280,overflowY:"auto"}}>
                      {t.data.length ? t.data.map(x=>(
                        <div key={x.ID} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"8px 0",borderBottom:"1px solid #1e3a5f",fontSize:12,gap:8}}>
                      ...
                      )):<div style={{color:"#475569",textAlign:"center",padding:30,fontSize:12}}>Tidak ada data</div>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab==="analitik"&&(
            <div>
              <h1 style={{fontSize:22,fontWeight:800,color:"#f1f5f9",marginBottom:20}}>📊 Analitik & Business Intelligence</h1>

              <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                <div className="card" style={{padding:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>NET PROFIT TREND</div>
                ...
                <div className="card" style={{padding:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:12}}>ANALISIS HARI TERBAIK</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:5}}>
                    {dayPred.map((d,i)=>(
                      <div key={i} style={{textAlign:"center",padding:"10px 4px",borderRadius:10,background:i===0?"rgba(56,189,248,.14)":i===1?"rgba(99,102,241,.09)":"rgba(30,58,95,.25)",border:`1px solid ${i===0?"#38bdf835":i===1?"#6366f125":"#1e3a5f30"}`}}>
                        ...
                        <div style={{fontSize:8,color:"#475569",marginTop:2}}>{d.count}×</div>
              ...
              {/* KPI */}
              <div className="card" style={{padding:18}}>
                <div style={{fontSize:11,fontWeight:700,color:"#64748b",letterSpacing:1,marginBottom:14}}>KEY PERFORMANCE INDICATORS</div>
                <div className="grid3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:11}}>
                  {[
                    ...
                    <div key={i} style={{padding:14,background:"#0a1628",borderRadius:10,border:"1px solid #1e3a5f"}}>
                      <div style={{fontSize:9,color:"#475569",fontWeight:700,marginBottom:6,letterSpacing:.5,textTransform:"uppercase"}}>{k.l}</div>
